# Hi there, I'm Charles Rodgers 👋

### 🖥️ IT Professional | Network Infrastructure & Systems Administration

I am an IT professional based in the Fort Worth metroplex, passionate about building reliable network infrastructure and maintaining enterprise systems. I am currently completing my B.S. in Cloud Computing at Purdue University Global (expected 2026) and actively documenting my homelab environments to bridge theoretical concepts with hands-on implementation. 

By utilizing my foundational network knowledge and troubleshooting expertise, I am pursuing Cloud Support Associate or **Remote IT Support** roles.

---

### 🛠️ What I'm Working On
- **Enterprise Homelabbing:** Deploying and configuring Windows Server 2022 and Linux environments.
- **Virtualization:** Setting up and managing virtual networks using VMware and Hyper-V.
- **Network Simulation:** Designing subnets and routing protocols in Cisco Packet Tracer.
- **Active Directory:** Implementing domain controllers, group policies, and user management.

---

### 📜 Certifications
Here are the industry credentials I hold:
*   **CompTIA Network+** (CompTIA IT Operations Specialist)
*   **CompTIA A+**
*   **Google IT Support Professional**
*   **Google Data Analytics**
*   **FOA Certified Fiber Optic Installer** (ETA)

---

### 💻 Tech Stack & Tools

**Systems & Servers:**
![Windows Server](https://img.shields.io/badge/Windows_Server-0078D6?style=for-the-badge&logo=windows&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Active Directory](https://img.shields.io/badge/Active_Directory-0078D7?style=for-the-badge&logo=microsoft&logoColor=white)

**Networking & Virtualization:**
![VMware](https://img.shields.io/badge/VMware-607078?style=for-the-badge&logo=vmware&logoColor=white)
![Cisco](https://img.shields.io/badge/Cisco-1BA0D7?style=for-the-badge&logo=cisco&logoColor=white)
![Hyper-V](https://img.shields.io/badge/Hyper--V-0078D7?style=for-the-badge&logo=microsoft&logoColor=white)

---

### ⚡ Outside of Tech
When I'm not configuring virtual switches or racking servers, you can usually find me meal-prepping for the week, optimizing the filtration on my high-performance aquarium setup, or keeping up with my 6-month-old puppy, Dandelion. 

---

### 📫 Let's Connect!
- **LinkedIn:** [linkedin.com/in/rodgerscharles](#) 
- **Email:** crworks1099@gmail.com 
